...
/**
 * Top navigation header component for the RO Plant management system.
 * @param {Object} props
 * @param {Object|null} props.user - The currently logged-in user object.
 * @param {Array} props.notifications - List of system notifications.
 * @param {Function} props.onLogout - Callback to handle session termination.
 */
const Header = ({ user, notifications, onLogout }) => {
    const [isNotificationsOpen, setNotificationsOpen] = useState(false);
    // TypeScript generic <HTMLDivElement> removed
    const notificationRef = useRef(null);

    // Calculate unread count using standard array filter
    const unreadCount = notifications.filter(n => !n.read).length;
    
    useEffect(() => {
        /**
         * Closes the notification dropdown if the user clicks outside of it.
         */
        const handleClickOutside = (event) => {
            // Type casting "as Node" removed
    ...
    return (
        <header className="h-20 bg-brand-surface border-b border-gray-200 flex items-center justify-end px-6">
            <div className="flex items-center space-x-6">
                ...
                <button 
                    onClick={onLogout} 
                    className="flex items-center text-brand-text-secondary hover:text-red-600 transition-colors"
                >
