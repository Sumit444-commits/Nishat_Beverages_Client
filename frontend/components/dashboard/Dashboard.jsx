...
import {
  inferPaymentCategoryFromInventory,
  is19LItemName,
  is6LItemName,
} from "../../utils/payment-category";
...
const Dashboard = ({ user, onLogout }) => {
  ...
  const [isRecordOpeningBalanceOpen, setRecordOpeningBalanceOpen] =
    useState(false);
  ...
  const fetchAllData = async () => {
    try {
      ...
      setDb(freshDb);
      return freshDb; // 💡 RETURN THE FRESH DATA SO HANDLERS CAN USE IT IMMEDIATELY
    } catch (error) {
  ...
  // Helper function to refresh the selected customer
  const refreshSelectedCustomer = (freshDb) => {
    if (selectedCustomer && freshDb) {
      const refreshedCustomer = freshDb.customers.find(
        (c) =>
          String(c._id || c.id) ===
  ...
  const notifications = useMemo(() => {
    const alerts = [];
    const lowStockItems = db.inventory.filter(
    ...
    recentSales.forEach((sale, index) => {
      ...
      const customerName = customerId
        ? db.customers.find((c) => String(c._id || c.id) === customerId)?.name
        : "Counter";
      alerts.push({
    ...
    return db.customers.filter((customer) => {
      ...
      const matchesStatus =
        ...
        (statusFilter === "pending" &&
          (Number(customer.totalBalance) || 0) > 0) ||
  ...
  const financialSummary = useMemo(() => {
    const openingBalanceRecord = (db.dailyOpeningBalances || []).find(
      (b) => b.date === financialDate,
    );
    const openingBalance = openingBalanceRecord
      ? (Number(openingBalanceRecord.cash) || 0) +
        (Number(openingBalanceRecord.bank) || 0)
      : 0;
    ...
    const totalSalesmanPaymentsToday = todaySalesmanPayments.reduce(
    ...
    const collection6L = todayPayments
      .filter((s) => {
        ...
        const item = sId
          ...
          : null;
        return (
          (item && is6LItemName(item.name)) ||
          s.paymentForCategory === "6Ltr Collection"
        );
      })
      .reduce((sum, s) => sum + (Number(s.amountReceived) || 0), 0);
  ...
  const handleAddCustomer = async (customerData) => {
    ...
    try {
      ...
      if (!finalSalesmanId && customerData.area) {
        const areaAssignment = (db.areaAssignments || []).find(
          (a) => a.area === customerData.area.trim(),
        );
        if (areaAssignment && areaAssignment.salesmanId) {
          finalSalesmanId = String(
            typeof areaAssignment.salesmanId === "object"
  ...
  const handleUpdateCustomer = async (updatedCustomer) => {
    ...
    try {
      ...
      const freshDb = await fetchAllData();
      refreshSelectedCustomer(freshDb);
    } catch (error) {
  ...
  const handleAddSale = async (saleData) => {
    ...
    try {
      ...
      const freshDb = await fetchAllData();
      refreshSelectedCustomer(freshDb); // 💡 FIX: Uses freshDb to update detail view!
    } catch (error) {
  ...
  // 💡 FIX: Make sure MarkAsPaid gets fresh customer data
  ...
  // 💡 FIX: Make sure RecordPayment gets fresh customer data
  ...
  // 💡 FIX: Ensure CollectEmpties gets fresh data
  const handleCollectEmpties = async (customerId, bottlesCollected) => {
    ...
    try {
      ...
      const newEmpties = Math.max(
        (Number(customer.emptyBottlesHeld) || 0) - Number(bottlesCollected),
        0,
      );
  ...
  const handleAddExpenseOwner = async (name) => {
    try {
      ...
      setDb((prev) => ({
        ...prev,
        expenseOwners: [...prev.expenseOwners, newOwner],
      }));
      return newOwner;
    } catch (error) {
  ...
  const handleUpdateExpense = async (updatedExpense) => {
    ...
    try {
      await apiService.updateExpense(
        updatedExpense._id || updatedExpense.id,
        updatedExpense,
      );
  ...
  const handleUpdateSalesman = async (updatedSalesman) => {
    setIsSyncing(true);
    try {
      await apiService.updateSalesman(
        updatedSalesman._id || updatedSalesman.id,
        updatedSalesman,
      );
  ...
  const handleUpdateDailyAssignment = async (updatedAssignment) => {
    setIsSyncing(true);
    try {
      await apiService.updateDailyAssignment(
        updatedAssignment._id || updatedAssignment.id,
        updatedAssignment,
      );
  ...
  const renderContent = () => {
    if (isInitializing) {
      return (
        <div className="flex flex-col items-center justify-center h-full">
          ...
          <p className="text-brand-text-secondary font-medium tracking-widest uppercase">
    ...
    switch (activeView) {
      case "Dashboard":
        const todayDeliveries = db.customers.filter((c) =>
          isDeliveryDue(c, db.sales),
        );
        const recentSales = [...db.sales]
          .sort(
            (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime(),
          ...
          <div className="animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
              <StatCard
                ...
                value={`PKR ${financialSummary.openingBalance.toLocaleString()}`}
                icon={<BriefcaseIcon />}
                color="text-indigo-500"
                onClick={() => setRecordOpeningBalanceOpen(true)}
              />
              <StatCard
                title="19Ltr Collection"
                value={`PKR ${financialSummary.collection19L.toLocaleString()}`}
                icon={<PackageIcon />}
                color="text-brand-blue"
              />
              <StatCard
                title="6Ltr Collection"
                value={`PKR ${financialSummary.collection6L.toLocaleString()}`}
                icon={<PackageIcon />}
                color="text-brand-accent"
              />
              <StatCard
                title="Counter Sale"
                value={`PKR ${financialSummary.counterSale.toLocaleString()}`}
                icon={<UsersIcon />}
                color="text-teal-500"
              />
              <StatCard
                title="Total Revenue (Today)"
                value={`PKR ${financialSummary.totalRevenueToday.toLocaleString()}`}
                icon={<DollarSignIcon />}
                color="text-green-500"
              />
              <StatCard
                title="Day's Est. Closing"
                value={`PKR ${financialSummary.grandTotal.toLocaleString()}`}
                icon={<TrendingUpIcon />}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden flex flex-col h-[400px]">
                <div className="p-5 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
                  <h2 className="text-lg font-black text-brand-text-primary">
                    Today's Deliveries
                  </h2>
                  <span className="text-xs font-bold bg-blue-100 text-brand-blue px-2.5 py-1 rounded-full">
                    {todayDeliveries.length} Routes
                  </span>
                </div>
                <div className="p-2 space-y-1 overflow-y-auto flex-grow">
                  {todayDeliveries.length > 0 ? (
                    todayDeliveries.map((c) => (
                      <div
                        key={c._id || c.id}
                        className="flex justify-between items-center p-3 rounded-lg hover:bg-gray-50 border border-transparent hover:border-gray-200 transition-all group"
                      >
                        ...
                        <button
                          ...
                          className="text-xs font-bold text-brand-blue bg-blue-50 px-3 py-1.5 rounded-md opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0"
                        >
                          Profile &rarr;
                        </button>
                      </div>
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {recentSales.map((s) => {
                          const customerName = s.customerId
                            ...
                            : "Counter Sale";
                          const itemName = s.inventoryItemId
                            ? db.inventory.find(
                                (i) =>
                                  String(i._id || i.id) ===
                                  String(
                                    typeof s.inventoryItemId === "object"
                                      ? s.inventoryItemId._id
                                      : s.inventoryItemId,
                                  ),
                              )?.name
                            : s.description;
                          ...
                          return (
                            ...
                            >
                              <td className="px-5 py-4 font-bold text-brand-text-primary truncate max-w-[120px]">
                                {customerName}
                              </td>
                              <td className="px-5 py-4 text-xs font-medium text-gray-500 truncate max-w-[100px]">
                                {itemName}{" "}
                                {s.quantity > 0 ? `(x${s.quantity})` : ""}
                              </td>
        ...
        return (
          <div className="animate-fade-in">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
              <h1 className="text-3xl font-black text-brand-text-primary tracking-tight">
                Customer Accounts{" "}
                <span className="text-lg text-brand-text-secondary font-medium ml-2">
                  ({filteredCustomers.length})
                </span>
              </h1>
              <button
                onClick={() => setAddCustomerOpen(true)}
                className="flex items-center justify-center bg-brand-blue text-white px-5 py-2.5 rounded-lg font-bold shadow-md hover:bg-brand-lightblue active:scale-95 transition-all w-full md:w-auto"
              >
            ...
            <CustomerAccounts
              ...
              onCollectEmpties={(customer) => {
                setSelectedCustomer(customer);
                setCollectEmptiesOpen(true);
              }}
      ...
      case "CustomerDetail":
        ...
        return (
          <CustomerDetail
            ...
            reminders={db.dailyReminders.filter(
              (r) =>
                String(r.customerId) ===
                String(selectedCustomer._id || selectedCustomer.id),
            )}
        ...
        return (
          <AreaAssignment
            ...
            onNavigateToSalesman={(salesmanId) => {
              ...
              const salesman = db.salesmen.find(
                (s) => String(s._id || s.id) === targetId,
              );
              if (salesman) {
                setSelectedSalesman(salesman);
                setActiveView("SalesmanDetail");
      ...
      case "SalesmanReport":
        if (!selectedSalesman)
          return <div>No salesman selected for report.</div>;
        return (
          <SalesmanDailyReport
            salesman={selectedSalesman}
            customers={db.customers}
        ...
        return (
          <InventoryDetail
            ...
            sales={db.sales.filter((s) => {
              const sId = String(s.inventoryItemId?._id || s.inventoryItemId);
              const iId = String(
                selectedInventoryItem._id || selectedInventoryItem.id,
              );
              return sId === iId;
            })}
            customers={db.customers}
      ...
      case "Outstanding":
        return (
          <Outstanding
            sales={db.sales}
            inventory={db.inventory}
            customers={db.customers.filter(
              (c) => (Number(c.totalBalance) || 0) > 0,
            )}
          />
        );
  ...
  return (
    <div className="flex h-screen bg-brand-bg relative">
      ...
      <RecordOpeningBalanceModal
        ...
        currentBalance={(db.dailyOpeningBalances || []).find(
          (b) => b.date === financialDate,
        )}
      />
      <ConfirmClosingModal
