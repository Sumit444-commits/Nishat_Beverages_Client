...
/**
 * Modal component for adding a sale record for a specific customer.
 * @param {Object} props
 * @param {boolean} props.isOpen - Visibility toggle.
 * @param {Function} props.onClose - Close handler.
 * @param {Function} props.onAddSale - Callback to save the new sale.
 * @param {Object|null} props.customer - The targeted customer object.
 * @param {Array} props.salesmen - List of available salesmen.
 */
const AddSaleModal = ({ isOpen, onClose, onAddSale, customer, salesmen }) => {
    ...
    // Sync form with customer data when modal opens or customer changes
    useEffect(() => {
        if (customer) {
    ...
    const handleSubmit = (e) => {
        ...
        // Constructing plain JS object matching your backend/db schema
        const newSale = {
            customerId: customer.id,
            salesmanId: salesmanId ? Number(salesmanId) : null,
            inventoryItemId: null, // Placeholder for generic bottle sales
            quantity,
            emptiesCollected,
            amount,
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4">
            <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label htmlFor="sale-date-simple" className="block text-sm font-medium text-brand-text-secondary">
                            Sale Date
                        </label>
                        <input
                            type="date"
                    ...
                    <div>
                        <label htmlFor="bottles-sold" className="block text-sm font-medium text-brand-text-secondary">
                            Bottles Sold
                        </label>
                        <input 
                            type="number" 
                     ...
                     <div>
                        <label htmlFor="empties-collected" className="block text-sm font-medium text-brand-text-secondary">
                            Empties Collected
                        </label>
                        <input 
                            type="number" 
                            id="empties-collected" 
                     ...
                     <div>
                        <label htmlFor="payment-method" className="block text-sm font-medium text-brand-text-secondary">
                            Payment Method
                        </label>
                        <select 
                            id="payment-method" 
                     ...
                     <div>
                        <label htmlFor="salesman-assign" className="block text-sm font-medium text-brand-text-secondary">
                            Assigned Salesman
                        </label>
                        <select 
                            id="salesman-assign" 
